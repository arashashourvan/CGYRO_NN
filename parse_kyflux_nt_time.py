...
def main():
    ...
    data = np.fromfile(path, dtype=np.float32)
    total = data.size
    ns, nm_expected, nf = 2, 3, 2

    if args.n_time <= 0:
    # try to infer n_time assuming m=3
       denom = ns * nm_expected * nf * args.n_n  # 12 * n_t
       if total % denom != 0:
           raise ValueError(f"Cannot infer n_time: total={total} not divisible by 12*n_t={denom}.")
       args.n_time = total // denom
       print(f"Inferred n_time = {args.n_time}")

    base = ns * nf * args.n_n * args.n_time  # 4 * n_t * n_time
    if total % base != 0:
        raise ValueError(f"Float count {total} is not divisible by 4*n_t*n_time = {base}.")
    m = total // base
